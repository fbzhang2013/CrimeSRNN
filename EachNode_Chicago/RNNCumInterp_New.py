...
Hourly, Daily, weekly fatures.
External features.
Note: here we use periodic superresolution, everyday there are 24 time slots, we add 23 betweem them.
"""
import numpy as np
...
def img_enlarge(img):
    ...
    #func = interp1d(x, img, kind='quadratic')
...
if __name__ == '__main__':
    ...
    numEvents3 = np.zeros((ndays*(TimeEachDay*2-1),))  #Periodically superresolve
    for i in range(ndays):
        tmp = img_enlarge(numEvents2[i*TimeEachDay:(i+1)*TimeEachDay])
        numEvents3[i*(TimeEachDay*2-1):(i+1)*(TimeEachDay*2-1)] = tmp
    ...
    Temp2 = Temp2[-numSlots1:]; Wind2 = Wind2[-numSlots1:]
    Events2 = Events2[-numSlots1:]; Holiday2 = Holiday2[-numSlots1:]; Time2 = Time2[-numSlots1:]
    print('External feature dimensions: ', len(Temp2), len(Wind2), len(Events2), len(Holiday2), len(Time2), len(numEvents3))
